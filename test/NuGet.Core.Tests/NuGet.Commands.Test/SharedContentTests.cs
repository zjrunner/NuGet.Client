...
namespace NuGet.Commands.Test
{
    public class SharedContentTests : IDisposable
    {
        ...
        public async Task SharedContent_DefaultActionsWithNoNuspecSettings()
        {
            ...
            var utilItem = target.Libraries.Single().SharedContentGroups
                .Single(group => group.PropertyValue == "cs")
                .Items
                .Single(item => item.Path == "shared/cs/net45/code/util.cs.pp");

            var configItem = target.Libraries.Single().SharedContentGroups
                .Single(group => group.PropertyValue == "cs")
                .Items
                .Single(item => item.Path == "shared/cs/net45/config/config.xml");

            var imageItem = target.Libraries.Single().SharedContentGroups
                .Single(group => group.PropertyValue == "cs")
                .Items
                .Single(item => item.Path == "shared/cs/net45/images/image.jpg");

            var utilFSItem = target.Libraries.Single().SharedContentGroups
                .Single(group => group.PropertyValue == "fs")
                .Items
            ...
            Assert.Equal(3, utilItem.Properties.Count);
        ...
        public async Task SharedContent_TurnOffSharedContent()
        {
            ...
            var count = target.Libraries.Single().SharedContentGroups.Count;
        ...
        public async Task SharedContent_EmptyFolder()
        {
            ...
            var item = target.Libraries.Single().SharedContentGroups
                .Single(group => group.PropertyValue == "cs")
                .Items
                .Single();
        ...
        public async Task SharedContent_CopyToOutputSettings()
        {
            ...
            var helperCsItem = target.Libraries.Single().SharedContentGroups
                .Single(group => group.PropertyValue == "cs")
                .Items
                .Single(item => item.Path == "shared/cs/net45/config/config.xml");
            ...
            Assert.Equal(0, logger.Errors);
            Assert.Equal(0, logger.Warnings);
            Assert.Equal(3, helperCsItem.Properties.Count);
            ...
            using (var zip = new ZipArchive(File.Create(file.FullName), ZipArchiveMode.Create))
            {
                ...
                zip.AddEntry("packageA.nuspec", @"<?xml version=""1.0"" encoding=""utf-8""?>
                        ...
                        <metadata>
                            ...
                            <title />
                            <shared>
                                <files include=""shared/cs/net45/config/config.xml"" buildAction=""none"" />
                                <files include=""shared/cs/net45/config/config.xml"" copyToOutput=""true"" flatten=""false"" />
                                <files include=""shared/cs/net45/images/image.jpg"" buildAction=""embeddedresource"" />
                            </shared>
                        </metadata>
                        </package>", Encoding.UTF8);
            }
