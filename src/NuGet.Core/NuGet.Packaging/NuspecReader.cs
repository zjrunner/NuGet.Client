...
namespace NuGet.Packaging
{
    ...
    public class NuspecReader : NuspecCoreReaderBase
    {
        ...
        /// Build action groups
        /// </summary>
        public IEnumerable<SharedFilesEntry> GetSharedFiles()
        {
            var ns = MetadataNode.GetDefaultNamespace().NamespaceName;

            foreach (var filesNode in MetadataNode
                .Elements(XName.Get(Shared, ns))
                .Elements(XName.Get(Files, ns)))
            {
                ...
                yield return new SharedFilesEntry(include, exclude, buildAction, copyToOutput, flatten);
            }
